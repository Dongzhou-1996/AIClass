...
def plt_oneplot(lst, title, ylabel, legend, save_dir, xlabel='step', color=[(1,0,0), (0,1,0), (0,0,1), (0,0,0)],
    ...
    if subplot:
        ...
        ax_inset.set_xlim(zoom_range_x[0], zoom_range_x[1]), ax_inset.set_ylim(zoom_range_y[0], zoom_range_y[1]), ax_inset.grid()
...
def plt_multiplot(lst, title, save_dir, xlabel='step', ylabel='Orientation Angle, rad', color=[(1,0,0), (0,1,0), (0,0,1)],
...
def train_post_process(episode_num, episode_steps, tracking_success_times, env, episode_errors_list,
    ...
    if episode_num % args.fps_check_times == 0:
        t = time.time()
...
def DDPG_train_post_process(args, episode_num, dirs, env, agent, total_steps, state_norm, reward_scaling,
    ...
    update_time = int(total_steps / args.save_batch_size)

    tmp_reward_scaling = reward_scaling

    if update_time % args.model_save_freq == 0:
        model_path = os.path.join(dirs['log'], 'model_ep_{:03d}.pth'.format(episode_num))
        increment_data_path = os.path.join(dirs['increment'], 'data_{:03d}.pth'.format(update_time))
        tmp = env.sphere_reach_fail_prob
        agent.save_model(episode_num=episode_num, total_steps=total_steps, state_norm=state_norm,
                         reward_scaling=tmp_reward_scaling, reach_fail_prob=tmp,
                         update_time=update_time, model_path=model_path, )
...
def reward_error_plot(args, dirs, episode_num, stable_rewards_list, stable_errors_list, update_time):
    ...
    update_episode_len = int(args.batch_size / (args.max_episode_steps+1))
...
def evaluation_step_plot(args, dirs, step_error_list, step_jp_list, step_jv_list, step_base_ori_list,
    ...
    plt_subplot(lT(step_jp_list), 'Manipulator joint angles', xlabel='step', ylabel='Joint Position, rad',
                save_dir=os.path.join(dirs['step_jp'], 'StepJointPosition_{}_scene{}.png'.format(basename, episode_num)))
    plt_subplot(lT(step_jv_list), 'Manipulator joint angular velocities', xlabel='step', ylabel='Joint Velocity, rad/s',
                save_dir=os.path.join(dirs['step_jv'], 'StepJointVelocity_{}_scene{}.png'.format(basename, episode_num)))
    if base_orientation_plot:
        plt_multiplot(lT(step_base_ori_list), title='Base satellite orientation', save_dir=os.path.join(dirs['base_ori'],
...
def evaluation_step_plot_forpaper(args, dirs, step_error_list, step_jp_list, step_jv_list, step_base_pos_list,
    ...
    plt_subplot(lT(step_error_list)[:3], 'Manipulator joint angular velocities', xlabel='step',
                ...
                save_dir=os.path.join(dirs['step_pos_err'], 'StepPositionError_{}_scene{}.png'.format(basename, episode_num)),
    ...
    plt_oneplot(lT(step_error_list)[:3], 'Manipulator joint angles', xlabel='step', ylabel='Position Error, m',
                save_dir=os.path.join(dirs['step_pos_err'], 'StepPositionError_OnePlot_{}_scene{}.png'.format(basename, episode_num)),
                legend=['x', 'y', 'z', 'End-effector', ], figsize=(5, 4), settitle=False, lw=[1.5, 1.5, 1.5],
                color=[(232/255,0,11/255), (26/255,201/255,56/255), (2/255, 62/255, 1), (0.15,0.15,0.15)])

    plt_subplot([flipthres(lT(step_error_list)[4], 3), flipthres(lT(step_error_list)[5], 3), flipthres(lT(step_error_list)[6], 3)],
                'Manipulator joint angular velocities', xlabel='step',
                ylabel=['Pitch-Error, rad', 'Roll-Error, rad', 'Yaw-Error, rad', ],
                save_dir=os.path.join(dirs['step_ori_err'], 'StepOrientationError_{}_scene{}.png'.format(basename, episode_num)),
    ...
    plt_oneplot([flipthres(lT(step_error_list)[4], 3), flipthres(lT(step_error_list)[5], 3),
                ...
                save_dir=os.path.join(dirs['step_ori_err'], 'StepOrientationError_OnePlot_{}_scene{}.png'.format(basename, episode_num)),
    ...
    if base_orientation_plot:
        ...
        plt_multiplot(lT(tmp), title='Base satellite orientation', save_dir=os.path.join(dirs['base_ori'],
                                'StepBaseOrientation_{}_scene{}.png'.format(basename, episode_num)),
                      figsize=(5, 3.5),
                      color=[(232/255,64/255,139/255), (120/255,201/255,92/255), (66/255, 126/255, 1), (0.15, 0.15, 0.15)],
                      label=['Pitch', 'Roll', 'Yaw', 'Base spacecraft'], linestyle=['-', '-', '-', '-'], lw=[1.5, 1.5, 1.5, 2],
                      t=0.97, b=0.12)

        tmp = step_base_pos_list
        # for i in range(len(tmp)):
        #     tmp[i].append((tmp[i][0]**2 + tmp[i][1]**2 + tmp[i][2]**2) ** 0.5)
        plt_multiplot(lT(tmp), ylabel='Position, m', title='Base satellite position', save_dir=os.path.join(dirs['base_ori'],
